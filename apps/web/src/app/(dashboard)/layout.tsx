...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-surface text-ink">
      ...
      {/* ============================== MAIN ============================== */}
      <main className="flex-1 flex flex-col overflow-x-auto bg-surface">
        {/* Topbar */}
        <header className="sticky top-0 z-20 backdrop-blur-md bg-surface/80 border-b border-line-soft">
          <div className="flex items-center justify-between px-6 py-3.5">
            <div className="flex items-center gap-3 min-w-0">
              <div className="min-w-0">
                <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.18em] text-ink-muted">
                  <span>{data?.tenant.name ?? 'Pousada'}</span>
                  <ChevronRight className="w-3 h-3" />
                  <span className="text-ink-soft truncate">{pageMeta.title || 'Painel'}</span>
                </div>
                <h1 className="font-serif text-xl tracking-serif text-ink mt-0.5 truncate">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={openCmdk}
                className="hidden sm:inline-flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-ink-soft hover:text-ink hover:bg-surface-sunken border border-line transition-all"
              >
